...
def create_chart_stock_pool_performance():
    ...
    fig, ax = plt.subplots(figsize=(8, 4.5))
    ax.plot(range(len(months)), portfolio, 'b-', linewidth=2, label='清数智算组合（评分≥65）')
    ...
    ax.set_title('清数智算选股组合 vs 沪深300（回测期：2024.1-2026.3）', fontsize=13, fontweight='bold', pad=12)
    ax.legend(fontsize=10)
    ax.grid(alpha=0.3)
...
def generate_business_plan():
    ...
    add_body_text(doc, '清数智算量化投资分析平台（以下简称"清数智算"）是一款面向A股和港股市场的开源智能投资分析辅助系统。平台的核心思路是：用程序化的方法替代人工逐只翻看，用多维度量化评分模型对全市场股票进行系统化筛选和评级，帮助投资者快速定位值得关注的标的，减少信息搜集和初步筛选的时间成本。')

    add_body_text(doc, '具体来说，清数智算的核心功能包括以下几个方面：第一，基于自研"五维评分模型"（质量、估值、成长、趋势、风险，满分100分）对全市场股票进行量化打分，自动生成BUY/ADD/WATCH/REDUCE/SELL五级投资信号；第二，内置策略回测引擎，用户可以设定选股条件和持仓周期，基于真实历史数据模拟投资组合的表现，查看总收益率、年化收益率、最大回撤、夏普比率等关键指标；第三，提供针对不同风险偏好的风格化投资策略报告——稳健型（低波动高分红蓝筹为主、最大仓位60%）、进取型（高成长强趋势个股为主、最大仓位90%）和保守型（低估值高安全边际个股为主、最大仓位40%）；第四，支持通过QQ邮箱和飞书Webhook自动推送每日信号摘要和风险预警。')

    add_body_text(doc, '与市场现有产品相比，清数智算的差异化定位在于：第一，开源免费——核心功能完全免费开放，代码托管在GitHub上，采用MIT协议，任何人可以自由使用和修改；第二，专注中长线基本面选股——不做短线技术分析，而是从公司质地、估值水平、成长能力等基本面维度进行中长期视角的选股分析；第三，一体化分析闭环——从数据采集、量化评分、信号生成、策略回测到报告输出，全流程在一个平台内完成，用户无需在多个工具之间切换。')

    add_body_text(doc, '本计划书将从市场环境、产品设计、商业模式、财务预测和风险管控等维度，对清数智算项目进行全面的分析和规划。项目计划融资500万元人民币，主要用于产品研发、市场推广和运营支出，目标在运营第三年实现盈亏平衡并开始规模化盈利。')
    ...
    add_body_text(doc, '清数智算科技有限公司（筹）计划注册于XX市高新技术产业开发区，是一家专注于金融科技领域的创业企业。公司名称"清数智算"取"金清数智算接"之意，寓意在普通个人投资者与专业量化分析方法之间搭建一座桥梁。')

    add_body_text(doc, '公司的核心产品——清数智算量化投资分析平台，是一个基于Python（FastAPI后端）和Next.js（React前端）技术栈构建的B/S架构应用。系统通过AKShare、Yahoo Finance和东方财富三个数据源获取A股和港股的行情数据与财务数据，运用自研的量化评分模型进行分析处理，最终以可视化的仪表盘、信号列表、分析报告等形式呈现给用户。系统严格遵守中国证监会相关规定，定位为"研究与辅助分析工具"，所有页面均标注"本系统仅供研究和辅助分析使用，不构成任何投资建议"的免责声明，不接入券商交易接口，不执行任何实际交易操作。')
    ...
    add_body_text(doc, '清数智算的目标市场可以从以下三个层次来理解：')
    ...
    add_body_text(doc, '清数智算的竞争策略是聚焦于一个尚未被充分满足的细分需求：面向不会编程的个人投资者，提供比简单选股工具更深入、比量化编程平台更易用、比金融终端更便宜的中长线基本面选股分析工具。具体来说：')
    add_body_text(doc, '相比同花顺等传统终端：清数智算的基础功能完全免费，专业版年费（998元）仅为传统终端年费（数千至数万元）的十分之一甚至更低。同时，清数智算的界面设计更加简洁，聚焦于选股分析这一核心场景，避免了传统终端功能过多导致的信息过载问题。')
    add_body_text(doc, '相比聚宽等量化编程平台：清数智算不需要用户编写任何代码，所有分析功能通过图形界面操作即可完成。虽然在策略灵活性上不如编程平台，但对于"选好股、拿住股"这一中长线投资需求来说已经足够。')
    add_body_text(doc, '相比问财等简单选股工具：清数智算提供了更完整的分析闭环——不仅能够选股，还能对选出的标的进行深度分析、回测验证和持续跟踪，帮助用户从"选出股票"到"理解为什么选这只股票"再到"验证选股逻辑是否有效"。')
    ...
    add_body_text(doc, '清数智算平台是一个基于Web浏览器访问的投资分析辅助系统，用户无需安装任何客户端软件，通过浏览器即可使用所有功能。系统支持中文和英文两种语言界面。平台的核心功能模块如下：')
    ...
    add_body_text(doc, '清数智算平台采用前后端分离的现代化Web架构，技术选型遵循"成熟稳定、社区活跃、开发效率高"的原则。')
    ...
    add_body_text(doc, '在数据采集方面，系统采用"数据提供者"（Data Provider）设计模式，通过统一的接口抽象层屏蔽不同数据源的差异。AKShare作为主数据源提供A股和港股的行情、财务和公告数据；当AKShare因网络或接口调整暂时不可用时，系统自动切换到Yahoo Finance或东方财富的接口获取数据，确保数据服务的连续性。这种多源冗余的设计是清数智算平台在技术层面的一个重要特点。')

    add_sub_heading(doc, '4.3 五维评分选股模型')
    add_body_text(doc, '五维评分模型是清数智算平台最核心的分析引擎。该模型从五个维度对一只股票进行量化评估，满分100分，每个维度的权重和评分指标如下：')
    ...
    add_body_text(doc, '策略回测是清数智算平台的另一核心功能。回测的基本逻辑是：用户设定选股条件（如评分≥65分）和持仓周期（如每月调仓一次），系统基于真实的历史价格数据，模拟按此策略构建的投资组合在过去一段时间内的收益表现。')
    ...
    add_body_text(doc, '综合以上分析，清数智算平台与市场现有产品相比的核心差异化优势可以概括为以下几点：')
    add_body_text(doc, '（1）开源免费，使用门槛低。清数智算采用MIT开源协议，核心代码完全公开在GitHub上，基础功能永久免费使用。用户无需支付数千至数万元的年费，也无需具备编程基础，通过浏览器即可使用全部分析功能。对于想要体验量化分析方法但预算有限的个人投资者来说，清数智算提供了一个零成本的入门选择。')
    add_body_text(doc, '（2）专注中长线基本面选股。与市场上大多数侧重短线技术分析的工具不同，清数智算的分析视角是中长线的基本面分析——关注公司的盈利能力、估值水平、成长趋势等内在价值因素，而非短期的价格波动和量价关系。这一定位与价值投资和长期投资的理念更为契合。')
    add_body_text(doc, '（3）一体化分析闭环。从数据采集到量化评分，从信号生成到策略回测，从报告输出到消息推送，清数智算在一个平台内覆盖了投资分析的完整流程。用户无需在行情软件、Excel表格、量化平台等多个工具之间来回切换，大大提升了分析效率。')
    add_body_text(doc, '（4）多数据源冗余保障。平台同时接入AKShare、Yahoo Finance和东方财富三个数据源，当主数据源出现问题时自动切换备用源，确保数据服务不中断。这种设计在同类开源项目中较为少见，是清数智算在技术可靠性方面的一个重要优势。')
    ...
    add_body_text(doc, '清数智算采用"基础免费+增值服务"的SaaS（软件即服务）商业模式，通过功能分层实现用户增长与商业变现的平衡。具体收入来源包括以下四个方面：')
    ...
    add_body_text(doc, '清数智算采用三级版本定价策略：')
    ...
    add_body_text(doc, '（2）开源社区运营：清数智算的代码托管在GitHub上，通过积极参与开源社区建设（撰写高质量的README文档、及时回复Issue、接受Pull Request等），吸引开发者和技术背景的投资者关注和使用。开源社区用户的口碑传播是低成本获客的重要渠道。')
    ...
    add_body_text(doc, '清数智算的品牌定位是"个人投资者的开源量化分析伙伴"。这一定位强调三个关键信息：面向个人投资者（而非机构）、开源开放（而非封闭收费）、量化分析辅助工具（而非投资建议服务）。品牌传播的核心理念是：用数据和方法论帮助投资者做出更好的决策，而不是替投资者做决策。')
    ...
    add_body_text(doc, '在内容营销方面，计划打造"清数智算研究院"品牌，定期发布以下类型的内容：每日市场速评（300-500字，简要点评当日市场热点和信号变化）；周度选股策略（1000-2000字，基于评分模型筛选本周值得关注的标的）；月度深度分析（3000-5000字，对某个行业或投资主题进行深入分析）；季度回测报告（对近期策略表现的回顾和优化方向）。通过持续的高质量内容输出，在量化投资分析领域建立专业形象。')
    ...
    add_body_text(doc, '在品牌建设方面，我们将通过以下方式建立清数智算的专业形象：第一，坚持产品页面的设计风格统一、信息层次清晰，给用户留下专业可靠的第一印象。第二，在所有对外内容中保持严谨的数据引用习惯，使用公开数据时注明来源，不夸大产品功能，不承诺投资收益。第三，积极参与行业交流活动，如量化投资论坛、金融科技峰会等，与同行交流学习，扩大品牌影响力。第四，在GitHub等开源社区保持活跃，及时回应用户反馈和建议，建立良好的开发者关系。')
    ...
    add_body_text(doc, '清数智算平台的日常运营以"数据驱动、自动化优先"为原则。核心运营流程如下：')
    ...
    add_body_text(doc, '金融合规方面：清数智算严格遵守中国证监会关于证券投资咨询业务的相关监管规定。平台在所有页面的显著位置标注"本系统仅供研究和辅助分析使用，不构成任何投资建议"的免责声明。平台不接入任何券商的交易接口，不执行任何实际的股票买卖操作，不收取任何形式的投资收益分成或利润分成。平台的数据来源均为公开的市场数据和财务数据，不涉及内幕信息或非公开信息。')
    ...
    add_body_text(doc, '清数智算项目面临的主要风险及应对策略如下：')
    ...
    add_body_text(doc, '清数智算项目的发展规划分为三个阶段，每个阶段都有明确的里程碑和关键绩效指标：')
    ...
    add_body_text(doc, '第三阶段（36-60个月）的核心任务是"建生态"。推出API开放平台，吸引量化开发者基于清数智算的数据和评分构建自己的策略；拓展至港股通、基金分析等新领域；探索企业版和定制化服务。目标是形成一个以清数智算评分体系为核心的量化投资工具生态。')

    add_body_text(doc, '清数智算团队将始终坚持"小步快跑、快速迭代"的创业节奏，在每个阶段结束后进行全面复盘和战略调整。我们有信心通过持续的技术创新和用户价值创造，将清数智算打造成为中国个人投资者信赖的智能分析工具。')
    ...
    add_body_text(doc, '在社会责任方面，清数智算将积极践行企业社会责任：第一，定期举办免费的投资者教育讲座和线上课程，普及基本的投资分析知识，帮助普通投资者提升金融素养，避免盲目跟风和情绪化交易。第二，与高校合作设立量化投资实习基地，为金融科技领域培养后备人才。第三，在产品设计中融入ESG（环境、社会和公司治理）投资理念，引导投资者关注企业的可持续发展能力。第四，承诺每年将净利润的1%捐赠给金融教育公益组织，支持金融普惠事业。')
    ...
    # 附录A：清数智算系统功能截图说明
    # ═══════════════════════════════════════
    add_heading_styled(doc, '附录A：清数智算系统功能截图说明')

    add_body_text(doc, '以下为清数智算量化投资分析平台的主要功能模块说明，基于系统实际开发情况进行描述。')
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('清数智算科技团队  |  2026年6月')
    run.font.name = FONT_SONGTI
    run._element.rPr.rFonts.set(qn('w:eastAsia'), FONT_SONGTI)
    run.font.size = SIZE_WU
    run.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
    ...
    # ─── 保存 ───
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '清数智算量化投资分析平台_创业计划书.docx')
