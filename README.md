# 融衔 (RongXian)

<div align="center">

**A股 + 港股中长期基本面选股与交易信号报告系统**

[![Python](https://img.shields.io/badge/Python-3.11+-blue.svg)](https://www.python.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.115+-green.svg)](https://fastapi.tiangolo.com/)
[![Next.js](https://img.shields.io/badge/Next.js-14+-black.svg)](https://nextjs.org/)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-16+-blue.svg)](https://www.postgresql.org/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

</div>

> **免责声明**：本系统仅用于研究和辅助分析，不构成任何投资建议。投资有风险，入市需谨慎。

## 产品定位

这是一个"金融分析 Agent"，不直接下单，只生成：

1. 市场状态判断
2. 中长期基本面选股信号
3. 个股分析报告
4. 组合仓位建议
5. 风险预警
6. 回测结果
7. **风格化投资策略报告（稳健/进取/保守）**
8. **模拟买入回测**

核心逻辑：**基本面筛选 → 估值判断 → 趋势确认 → 生成报告**

## 技术栈

| 层       | 技术                                   |
| -------- | -------------------------------------- |
| 前端     | Next.js 14 + TypeScript + Tailwind CSS |
| 图表     | Recharts                               |
| 后端     | FastAPI + Python 3.11                  |
| 数据库   | PostgreSQL 16 / SQLite（开发）         |
| ORM      | SQLAlchemy 2.0                         |
| 定时任务 | APScheduler                            |
| 数据处理 | Pandas / NumPy                         |
| PDF生成  | xhtml2pdf                              |
| 缓存     | Redis（可选）                          |
| 部署     | Docker Compose                         |

## 快速启动

### 方式一：Docker Compose（推荐）

```bash
cd 融资衔
cp .env.example .env
docker-compose up --build

# 前端: http://localhost:3000
# 后端 API 文档: http://localhost:8000/docs
```

### 方式二：本地开发

#### 后端

```bash
cd backend

python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

pip install -r requirements.txt

# 初始化数据库并导入 Mock 数据
python -m app.seed

# 启动后端服务
uvicorn app.main:app --reload --port 8000
```

#### 前端

```bash
cd frontend

npm install
npm run dev

# 访问 http://localhost:3000
```

## 功能模块

### 1. 仪表盘 (`/dashboard`)

- 市场指数概览
- 策略摘要（市场状态、建议仓位）
- 热门信号 TOP 5
- 信号分布图
- 股票池快览
- 风险预警

### 2. 信号中心 (`/signals`)

- 按市场/类型/评分筛选信号
- 信号详情（入场价、目标价、止损价）
- 信号强度和建议仓位

### 3. 股票分析 (`/stocks/{symbol}`)

- 行情数据（K线、成交量）
- 技术指标（MA、MACD、RSI）
- 财务数据（营收、利润、ROE）
- 评分详解（五维雷达图）
- 同业比较
- 操作建议

### 4. 策略总览 (`/pools`)

8种股票池：

| 池类型 | 说明 | 筛选条件 |
|--------|------|----------|
| 质量优选 | 高质量公司 | quality_score ≥ 22 |
| 估值洼地 | 低估值标的 | valuation_score ≥ 15 |
| 趋势向好 | 技术面强势 | trend_score ≥ 15 |
| 风险警示 | 高风险标的 | risk_score < 5 |
| **稳健优选** | 低波动蓝筹 | quality ≥ 20, risk ≥ 6, PE < 60 |
| **进取优选** | 高成长标的 | growth ≥ 15, trend ≥ 15 |
| **保守优选** | 高安全边际 | valuation ≥ 15, quality ≥ 18 |
| **周波动>2%** | 近5日活跃标的 | 周振幅 ≥ 2% |

### 5. 报告中心 (`/reports`)

#### 系统报告
- **每日策略报告**：市场综述、信号分布、重点推荐、风险预警、操作建议
- **个股深度分析**：行情、技术面、财务、估值、评分、同业比较
- **风格化策略报告**（三种风格）：

| 风格 | 特点 | 仓位上限 | 适合人群 |
|------|------|----------|----------|
| 🛡️ 稳健型 | 低波动、高股息、蓝筹优先 | 60% | 风险厌恶型、退休人群 |
| 🚀 进取型 | 高成长、强趋势、超额收益 | 90% | 年轻人、专业投资者 |
| 🏦 保守型 | 低估值、高安全、本金优先 | 40% | 大资金保守配置 |

#### 券商研报
- 东方财富研报数据
- 按股票代码/名称搜索
- 盈利预测（EPS/PE）

#### PDF下载
- 所有报告支持一键下载PDF
- 专业A4排版，含封面页

### 6. 回测中心 (`/backtest`)

#### 策略回测
- 基于评分的选股策略回测
- 权益曲线（策略 vs 基准）
- 月度超额收益柱状图
- 核心指标：总收益、年化收益、超额收益、最大回撤、夏普比率

#### 模拟买入
- 输入持仓列表（股票代码 + 买入日期 + 股数）
- 用真实历史价格计算收益
- 个股盈亏明细
- 组合权益曲线和月度收益对比

### 7. 管理后台 (`/admin`)

- 系统统计（股票数、信号数、用户数）
- 用户管理（角色、状态）
- 数据库表浏览

## 评分模型

总分 100 分：

```
total_score = quality_score (30) + valuation_score (20) + growth_score (20) + trend_score (20) + risk_score (10)
```

### 评级规则

| 分数区间 | 评级   | 含义 |
| -------- | ------ | ---- |
| ≥ 85    | BUY    | 买入 |
| 75-84    | ADD    | 加仓 |
| 65-74    | WATCH  | 观察 |
| 50-64    | REDUCE | 减仓 |
| < 50     | SELL   | 卖出 |

## 信号生成规则

- **BUY**: 总分 ≥ 85, 质量 ≥ 24, 估值 ≥ 14, 趋势 ≥ 14
- **ADD**: 总分 ≥ 75, 趋势确认, 风险分 ≥ 7
- **WATCH**: 基本面好但趋势未确认
- **REDUCE**: 估值过高 / 趋势转弱 / 风险升高
- **SELL**: 基本面恶化 / 评分极低

## 项目结构

```
├── backend/
│   ├── app/
│   │   ├── api/              # API 路由
│   │   │   ├── dashboard.py  # 仪表盘 API
│   │   │   ├── stocks.py     # 股票 API
│   │   │   ├── signals.py    # 信号 API
│   │   │   ├── pools.py      # 股票池 API（含风格池+波动池）
│   │   │   ├── reports.py    # 报告 API（含风格报告+PDF下载）
│   │   │   ├── backtest.py   # 回测 API（含模拟买入）
│   │   │   ├── auth.py       # 认证 API
│   │   │   ├── settings.py   # 设置 API
│   │   │   └── admin.py      # 管理 API
│   │   ├── models/           # 数据库模型
│   │   ├── schemas/          # Pydantic 模式
│   │   ├── services/         # 业务逻辑
│   │   │   ├── scoring.py    # 评分模型
│   │   │   ├── signal.py     # 信号生成
│   │   │   ├── report.py     # 报告生成（含风格化报告）
│   │   │   ├── backtest.py   # 回测服务（含模拟买入）
│   │   │   ├── pdf_service.py # PDF 生成服务
│   │   │   └── dashboard.py  # 仪表盘聚合
│   │   ├── data_providers/   # 数据源适配器
│   │   ├── core/             # 核心配置
│   │   ├── db/               # 数据库连接
│   │   ├── seed.py           # 种子数据脚本
│   │   └── main.py           # FastAPI 入口
│   ├── Dockerfile
│   └── requirements.txt
├── frontend/
│   ├── src/
│   │   ├── app/              # Next.js 页面
│   │   │   ├── dashboard/    # 首页仪表盘
│   │   │   ├── stocks/       # 股票分析
│   │   │   ├── signals/      # 信号中心
│   │   │   ├── pools/        # 策略总览（8种池）
│   │   │   ├── reports/      # 报告中心（风格报告+PDF）
│   │   │   ├── backtest/     # 回测中心（模拟买入）
│   │   │   ├── settings/     # 系统设置
│   │   │   └── admin/        # 管理后台
│   │   ├── components/       # 公共组件
│   │   ├── lib/              # 工具函数
│   │   └── types/            # TypeScript 类型
│   ├── Dockerfile
│   └── package.json
├── infra/
│   └── init.sql              # 数据库初始化
├── docker-compose.yml
└── .env.example
```

## API 接口文档

启动后端后访问：http://localhost:8000/docs

### 核心接口

| 方法 | 路径                              | 说明               |
| ---- | --------------------------------- | ------------------ |
| GET  | `/api/dashboard`                | 仪表盘数据         |
| GET  | `/api/stocks/search?keyword=`   | 搜索股票           |
| GET  | `/api/stocks/{symbol}`          | 股票详情           |
| GET  | `/api/signals`                  | 信号列表           |
| GET  | `/api/pools?type=`              | 股票池（8种类型）  |
| GET  | `/api/reports`                  | 报告列表           |
| GET  | `/api/reports/{id}`             | 报告详情           |
| GET  | `/api/reports/{id}/pdf`         | 下载报告PDF        |
| POST | `/api/reports/generate`         | 生成报告（支持style参数） |
| POST | `/api/reports/generate-style`   | 生成风格化策略报告 |
| GET  | `/api/reports/research`         | 券商研报列表       |
| POST | `/api/backtest/run`             | 运行策略回测       |
| POST | `/api/backtest/simulate`        | 模拟买入回测       |

### 认证接口

| 方法 | 路径                  | 说明     |
| ---- | --------------------- | -------- |
| POST | `/api/auth/login`     | 用户登录 |
| POST | `/api/auth/register`  | 用户注册 |
| GET  | `/api/auth/me`        | 当前用户 |

## 后续规划

- [x] 接入真实数据源（AkShare / Yahoo Finance / 东方财富）
- [x] 完善回测引擎（月度超额收益、模拟买入）
- [x] 风格化投资策略报告（稳健/进取/保守）
- [x] PDF报告下载
- [x] 8种股票池（含风格池和波动池）
- [ ] **API密钥管理（管理员配置，控制所有用户的API调用）**
- [ ] 邮件/微信推送
- [ ] AI 报告生成（LLM）
- [ ] 多周期回测对比
- [ ] 组合管理（实时持仓跟踪）
